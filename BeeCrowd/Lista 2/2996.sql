...
select p.year, s.name as 'sender', r.name as 'receiver' from packages p
join users s on p.id_user_sender = s.id 
join users r on p.id_user_receiver = r.id 
where (p.color like 'blue' or p.year = 2015) and s.address not like 'Taiwan' and r.address not like 'Taiwan'
